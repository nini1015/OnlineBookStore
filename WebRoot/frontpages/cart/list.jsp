...
<html>
  <head>
	...
	<link rel="stylesheet" type="text/css" href="<c:url value='/frontpages/css/cart/list.css'/>">
	...
	$(".jian").click(function() {
		...
		if(quantity == 1) {
			if(confirm("您是否真要删除该条目？")) {
				location = "/OnlineBookstore/CartItemServlet?method=batchDelete&cartItemIds=" + id;
			}
		} else {
...
function sendUpdateQuantity(id, quantity) {
	$.ajax({
		async:false,
		cache:false,
		url:"/OnlineBookstore/CartItemServlet",
...
function batchDelete(event) {
	...
	if (cartItemIdArray == '') {
		...
		location.href = "/CartItemServlet?method=batchDelete&cartItemIds=" + cartItemIdArray;
	}
}
...
<c:choose>
	...
	<table width="95%" align="center" cellpadding="0" cellspacing="0">
		<tr>
			<td align="right">
				<img align="top" src="<c:url value='/images/cartempty.png'/>"/>
			</td>
			<td>
				<h2 class="spanEmpty">您的购物车还是空的！</h2>
			</td>
		</tr>
	</table>  
	</c:when>
...
<c:forEach items="${cartItemList }" var="cartItem">
	...
	<tr>
		<td colspan="4" class="tdBatchDelete">
			<!-- <a href="javascript:batchDelete(event);">批量删除</a> -->
		</td>
